...
class _ChatbotState extends State<Chatbot> {
  ...
  void _sendMessage(ChatMessage chatMessage) async {
    ...
    try {
      final response = await _getModelResponse(chatMessage.text);
  ...
  Future<String?> _getModelResponse(String userInput) async {
    final uri = Uri.parse('https://api.beta.bot/v1/models/01/response');  // URL del modelo entrenado de GEMINI
  ...
  void _sendMediaMessage() async {
    ...
    if (file != null) {
      ChatMessage chatMessage = ChatMessage(
        ...
        createdAt: DateTime.now(),
        text:
            'Describe esta imagen de mi receta medica y recuerdame tomar mis medicamentos',
        medias: [
          ChatMedia(
            url: file.path,
            fileName: "",
